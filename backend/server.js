...
async function loadModel() {
    if (fs.existsSync(modelPath)) {
        try {
            ...
            console.error("Corrupted model file detected. Deleting and retraining...");
...
function encodeInput(data) {
    const educationMap = { "Graduate": 1, "Not Graduate": 0, 1: 1, 0: 0, "1": 1, "0": 0 };
    const selfEmployedMap = { "Yes": 1, "No": 0, 1: 1, 0: 0, "1": 1, "0": 0 };
    return [
        parseInt(data.no_of_dependents),
        educationMap[data.education] !== undefined ? educationMap[data.education] : parseInt(data.education),
        selfEmployedMap[data.self_employed] !== undefined ? selfEmployedMap[data.self_employed] : parseInt(data.self_employed),
...
(async () => {
    ...
    app.post("/check_eligibility", (req, res) => {
        try {
            if (!model) {
                return res.status(503).json({ error: "Model is still training. Please try again in a moment." });
            }
